<script setup lang="ts">
    ...
    function handleInput(event: Event) {
        const inputEvent = event as InputEvent; // Convertir a InputEvent
...
<template>
    <div class="w-full flex flex-col px-8 py-5 bg-gray-100 rounded-lg box-border">
        ...
        <textarea 
            class="w-full rounded-lg bg-white px-3 py-2 min-h-24 my-3 box-border max-w-full"
            v-model="comment"
            @input="handleInput"    
        ></textarea>
        <div class="w-full flex justify-between items-center">
            <p class="text-xs text-red-600" v-if="isLimit">Reached max characters</p>
            <button @click="publishComment" class="text-sm font-semibold text-white px-3 py-1 rounded bg-sky-500 cursor-pointer active:scale-95">Publish</button>
        </div>
    </div>
</template>
